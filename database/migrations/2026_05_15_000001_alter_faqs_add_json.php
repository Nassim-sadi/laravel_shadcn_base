...
return new class extends Migration
{
    public function up(): void
    {
        ...
        DB::table('faqs')->whereNull('deleted_at')->orderBy('id')->each(function ($row) use ($defaultLocale) {
            $updates = [];
            foreach (['question', 'answer', 'seo_title', 'seo_description'] as $column) {
                if (! is_null($row->$column) && ! isJson($row->$column)) {
                    $updates[$column] = json_encode([$defaultLocale => $row->$column]);
                }
            }
            if (! empty($updates)) {
                DB::table('faqs')->where('id', $row->id)->update($updates);
            }
        });
    }
