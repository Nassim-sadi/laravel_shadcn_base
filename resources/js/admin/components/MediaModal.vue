...
import { XIcon, UploadIcon, ImageIcon, Loader2Icon, CheckIcon, Trash2Icon } from '@lucide/vue'
import { useGetMediaQuery, useUpdateMediaMutation, useDeleteMediaMutation } from '@/services/api/media.api'
...
function handleGridSelect(id: number) {
  ...
  if (item) selectedImage.value = item
}
        ...
        <div class="flex-1 overflow-y-auto p-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                  <div class="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                    ...
                    <div v-if="editingImage.width"><span class="font-medium">Dimensions:</span> {{ editingImage.width }}x{{ editingImage.height }}</div>
                  ...
                  <div class="flex gap-2 pt-2">
                    <Button class="flex-1" @click="saveEdit">Save Changes</Button>
          ...
          <div v-if="activeTab === 'upload'" class="space-y-6">
            ...
            >
              ...
              <p class="text-sm text-muted-foreground">Drag & drop files here or click to browse</p>
              <p class="text-xs text-muted-foreground mt-1">JPG, PNG, GIF, WebP, SVG, PDF, DOC, XLS, MP4</p>
            </div>
              ...
              >
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div v-if="entry.status === 'uploading' || entry.status === 'done'" class="mt-1.5">
                    ...
                    <p class="text-[10px] text-muted-foreground mt-0.5">{{ entry.progress }}%</p>
                  </div>

                  <p v-if="entry.status === 'error'" class="text-[10px] text-destructive mt-0.5">{{ entry.error }}</p>
                  <p v-else-if="entry.status === 'done'" class="text-[10px] text-green-600 mt-0.5">Uploaded</p>
                  <p v-else class="text-[10px] text-muted-foreground mt-0.5">{{ (entry.file.size / 1024).toFixed(0) }} KB</p>
                </div>
