...
function handleStatusChange(value: string) {
  const current = props.filters.is_published.value
  props.filters.is_published.value = current === value ? undefined : value
    ...
    <div class="flex items-center flex-1 space-x-2">
      ...
      <div class="relative">
        <Button
          ...
          size="sm"
          class="h-8 text-xs"
          :class="{ 'border-primary': filters.is_published.value !== undefined }"
          @click="showStatusDropdown = !showStatusDropdown"
        >
          Status
        </Button>
        <div
          v-if="filters.is_published.value !== undefined"
        ...
        <Teleport to="body">
          ...
          >
            <button
              ...
              :class="{ 'bg-accent': filters.is_published.value === option.value }"
              @click="handleStatusChange(option.value)"
            >
              <span class="size-4 rounded-sm border flex items-center justify-center">
                <span v-if="filters.is_published.value === option.value" class="size-2 rounded-sm bg-primary" />
              </span>
              {{ option.label }}
            </button>
          </div>
